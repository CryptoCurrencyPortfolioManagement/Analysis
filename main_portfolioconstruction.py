...
projects_base_names = [
    ...
    "bertRNNSeperateApproaches_pf_final",
    "bertRNNSeperateApproaches-RL"
]

projects_sweep_names = {
    "bertRNNSeperateApproaches_final": "9azvhzrd",
    "bertRNNSeperateApproaches_pf_final": "9qbrp3w7",
    "bertRNNSeperateApproaches-RL": "fyew2ofj"
}
...
for project_base_name in tqdm(projects_base_names, desc= "fetching data"):
    ...
    if "reward" in df.columns:
        df.loc[df["reward"].isnull(), "reward"] = "Return"
    ...
    if not os.path.exists("./output_portfolioconstruction/{}".format(project_base_name.replace("Approaches_final", ""))):
        os.mkdir("./output_portfolioconstruction/{}".format(project_base_name.replace("Approaches_final", "")))
    ...
    for metric in metrics:
        for independent_variable in independent_variables:
            ...
            plt.savefig("./output_portfolioconstruction/{}/plot_{}_{}.png".format(project_base_name.replace("Approaches_final", ""), metric, independent_variable))
            plt.close()
    ...
    for metric in metrics:
        ...
        plt.savefig("./output_portfolioconstruction/{}/plot_performance_{}.png".format(project_base_name.replace("Approaches_final", ""), metric))
        plt.close()
    del df_for_boxplots
    ...
    for metric in metrics:
        ...
        plt.xlim(-2, 3)
        plt.savefig("./output_portfolioconstruction/{}/plot_{}.png".format(project_base_name.replace("Approaches_final", ""), metric))
        plt.close()
        ...
        res = mod.fit()
        with open("./output_portfolioconstruction/{}/table_significance_{}.tex".format(project_base_name.replace("Approaches_final", ""), metric), "w") as f:
        ...
        res_df.to_csv("./output_portfolioconstruction/{}/table_significance_{}.csv".format(project_base_name.replace("Approaches_final", ""), metric))
        #plt.rc('figure', figsize=(10, 10))
        #plt.text(0.01, 0.05, str(res.summary()), {'fontsize': 10}, fontproperties='monospace')
        #plt.axis('off')
        #plt.tight_layout()
        #plt.savefig("./output_portfolioconstruction/{}/table_significance_{}.png".format(project_base_name, metric))
        #plt.close()
    del df_analysis
    ...
    for i in range(top_to_use):
        ...
        if len(raw_data) > 0:
            ...
            plt.savefig("./output_portfolioconstruction/{}/plot_positions_CV6_top{}.png".format(project_base_name.replace("Approaches_final", ""), i+1))
            plt.close()
            pd.DataFrame(position_array, columns=ccs).to_excel("./output_portfolioconstruction/{}/positions_CV6_top{}.xlsx".format(project_base_name.replace("Approaches_final", ""), i + 1))
        else:
        ...
        plt.tight_layout()
        plt.savefig("./output_portfolioconstruction/{}/plot_performance_top{}.png".format(project_base_name.replace("Approaches_final", ""), i))
        plt.close()
    ...
    for metric in metrics:
        ...
        plt.tight_layout()
        plt.savefig('./output_portfolioconstruction/{}/plot_{}_top10_runs.png'.format(project_base_name.replace("Approaches_final", ""), metric))
        plt.close()
    ...
    sources = list(top_history_data["source"].value_counts().index)
    for source in sources:
        #text_df = pq.ParquetDataset("data/Content.parquet", validate_schema=False, filters=[('cc', 'in', ccs), ('source', '=', source)])
        #text_df = text_df.read(columns=["content_raw"]).to_pandas()
        #text_df["date"] = pd.to_datetime(text_df["date"]).astype('datetime64[ns]').apply(lambda x: x.date())

        #content_per_test = []
        #content_per_test_source = []
        #content_per_train = []
        #content_per_train_source = []
        #for window in windows:
        #    content_per_test.extend(text_df["source"].loc[(text_df["date"] > window[0]) & (text_df["date"] < window[1])].value_counts().tolist())
        #    content_per_test_source.extend(text_df["source"].loc[(text_df["date"] > window[0]) & (text_df["date"] < window[1])].value_counts().index.tolist())
        #    content_per_train.extend(text_df["source"].loc[text_df["date"] < window[0]].value_counts().tolist())
        #    content_per_train_source.extend(text_df["source"].loc[text_df["date"] < window[0]].value_counts().index.tolist())
        #runs_in_history_data = top_history_data["run"].nunique()
        #plot_df = top_history_data.loc[:, ["index", "run", "sharpeRatio_CV", "source"]]
        #plot_df = plot_df.rename(columns = {'sharpeRatio_CV':'value'})
        #plot_df["variable"] = "sharpeRatio per run"
        #avg_top_history_data = top_history_data.loc[:, ["index", "sharpeRatio_CV", "source"]].groupby(["index", "source"]).mean()["sharpeRatio_CV"]
        #plot_df = plot_df.append(pd.DataFrame(data=map(list, zip(*[[i for i in range(int(len(avg_top_history_data.tolist())/len(sources))) for _ in range(len(sources))],
        #                                                           avg_top_history_data.reset_index().source.to_list(),
        #                                                           avg_top_history_data.tolist(),
        #                                                           ["avg sharpeRatio"] * len(avg_top_history_data.tolist())])),
        #                                      columns=["index", "run", "value", "variable"]), ignore_index=True)
        #plot_df = plot_df.append(pd.DataFrame(data= map(list, zip(*[[i for i in range(int(len(content_per_test)/len(sources))) for _ in range(len(sources))],
        #                                                            content_per_test_source ,
        #                                                            content_per_test,
        #                                                            ["texts in test set"]*len(content_per_test)])),
        #                                      columns=["index", "run", "value", "variable"]), ignore_index= True)
        #plot_df = plot_df.append(pd.DataFrame(data=map(list, zip(*[[i for i in range(int(len(content_per_train)/len(sources))) for _ in range(len(sources))],
        #                                                           content_per_train_source,
        #                                                           content_per_train,
        ...
        #plt.tight_layout()
        #plt.savefig("./output_portfolioconstruction/{}/plot_performanceVsAvailableData_avg{}_{}.png".format(project_base_name.replace("Approaches_final", ""), top_to_use, source))
        #plt.close()
        ...
        runs_in_history_data = top_history_data.loc[top_history_data["source"] == source]["run"].nunique()
        plot_df = top_history_data.loc[
            top_history_data["source"] == source, ["index", "run", "sharpeRatio_CV"]]
        plot_df = plot_df.rename(columns={'sharpeRatio_CV': 'value'})
        plot_df["variable"] = "sharpeRatio per run"
        avg_top_history_data = top_history_data.loc[top_history_data["source"] == source].loc[:, ["index", "sharpeRatio_CV"]].groupby("index").mean()["sharpeRatio_CV"].tolist()
        ...
        g.map(sns.lineplot, "index", "value")
        g.fig.suptitle('{}:\n performance vs available data\n ({} runs, {})\n'.format(
            project_base_name.replace("Approaches_final", ""), runs_in_history_data, source))
        plt.tight_layout()
        plt.savefig("./output_portfolioconstruction/{}/plot_performanceVsAvailableData_avg{}_{}.png".format(project_base_name.replace("Approaches_final", ""), top_to_use, source))
        plt.close()
    ...
    for sgnf_level, descriptor in zip([0.1, 0.05, 0.01, 0.001], [".", "*", "**", "***"]):
        res_df.loc[res_df["P>|t|"] < sgnf_level, "sgnf_stars"] = descriptor
    res_df.to_csv("./output_portfolioconstruction/{}_table_significance_performanceVsAvailableData_avg{}.csv".format(project_base_name.replace("Approaches_final", ""), top_to_use))
    ...
    plt.tight_layout()
    plt.savefig("./output_portfolioconstruction/{}/plot_performanceVsMarketTrend_avg{}.png".format(project_base_name.replace("Approaches_final", ""), top_to_use))
    plt.close()
    ...
    for sgnf_level, descriptor in zip([0.1, 0.05, 0.01, 0.001], [".", "*", "**", "***"]):
        res_df.loc[res_df["P>|t|"] < sgnf_level, "sgnf_stars"] = descriptor
    res_df.to_csv("./output_portfolioconstruction/{}_table_significance_performanceVsMarketTrend_avg_{}.csv".format(project_base_name.replace("Approaches_final", ""), top_to_use))
...
for top in tops:
    ...
    for metric in metrics:
        metric_std = results_df[metric].std()
        metric_mean = results_df[metric].mean()
        sns.boxplot(data= results_df.loc[(results_df[metric] < metric_mean + 3 * metric_std) & (results_df[metric] > metric_mean - 3 * metric_std)], x= "algorithm", y=metric).set(title='Comparison {} (Top {})'.format(metric, top))
        plt.xticks(rotation=90)
        plt.tight_layout()
        plt.savefig("./output_portfolioconstruction/plot_performanceComparison_{}_top{}.png".format(metric, top))
        plt.close()
